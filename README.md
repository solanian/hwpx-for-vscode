...
> **Disclaimer** — 이 확장은 한글과컴퓨터(Hancom)의 공식 제품이 아니며, 한글과컴퓨터와 어떠한 제휴·보증·후원 관계도 없습니다. HWPX는 공개 표준(OWPML, KS X 6101:2024)에 기반한 문서 형식이며, 이 확장은 해당 표준을 참조하여 독립적으로 개발되었습니다.
...
- 표 (셀 병합, 테두리, 배경색)
- 이미지 (BinData base64)
- 페이지 레이아웃 (용지 크기, 여백, 가로/세로 방향)
- 페이지 분할 (표 행 분할, 셀별 독립 클리핑 포함)
- 글머리 기호 / 번호 매기기 (PUA 문자 지원, autoIndent hanging indent)
- 이미지 크기 (문서 지정 크기 hp:curSz/hp:sz 반영)
- 대용량 이미지 지연 로딩 (Chromium 테이블 파싱 호환)
- 탭 정지, 스타일 상속
...
| `HWPX: Generate Cursor Instructions (.cursorrules)` | 워크스페이스 루트에 `.cursorrules` 생성. API 포트·토큰이 자동 주입됨 |
| `HWPX: Generate Kiro Instructions (.kiro/rules/hwpx-api.md)` | 워크스페이스에 `.kiro/rules/hwpx-api.md` 생성. API 포트·토큰이 자동 주입됨 |
...
| 페이지 레이아웃 (용지 크기, 여백, 가로/세로 방향) | Supported |
| 페이지 분할 (표 행 분할, 셀별 독립 클리핑) | Supported |
| 글머리 기호 / 번호 매기기 (PUA 문자, autoIndent) | Supported |
| 이미지 크기 (문서 지정 크기 반영) | Supported |
| 탭 정지 | Supported |
| 스타일 상속 | Supported |
...
2. `Ctrl+Shift+P` → 사용하는 도구에 맞는 커맨드 실행:
   ...
   - Cursor → `HWPX: Generate Cursor Instructions (.cursorrules)`
   - Kiro → `HWPX: Generate Kiro Instructions (.kiro/rules/hwpx-api.md)`
...
> **Note**: 확장이 재시작되면 포트와 토큰이 변경됩니다. 연결 오류(401 등) 발생 시 지침 생성 커맨드를 다시 실행하세요.
...
GitHub: [https://github.com/solanian/hwpx-for-vscode](https://github.com/solanian/hwpx-for-vscode)

버그 리포트, 기능 요청, PR은 GitHub Issues/PR을 이용해주세요.
...
| 라이브러리 | 버전 | 라이선스 | 용도 |
|-----------|------|---------|------|
| [JSZip](https://github.com/Stuk/jszip) | 3.10.1 | MIT OR GPL-3.0-or-later | HWPX(ZIP) 파일 읽기/쓰기 |
| [fast-xml-parser](https://github.com/NaturalIntelligence/fast-xml-parser) | 4.x | MIT | XML 파싱 및 빌드 |
